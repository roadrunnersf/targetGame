...
Trgt.prototype = {
  ...
  shrinkOrDiss: function() {
    if (this.s > 0) { //target not hit but bigger than 0
      this.shrink();
    } else { //target shrunk too small (DISSappeared) so reset target
      this.diss();
    }
  },
  shrink: function() {
  ...
  chooseColor: function() {
    if (hair.x >= this.x && hair.x <= (this.x + this.s) && hair.y >= this.y && hair.y <= (this.y + this.s)) {
      //inside target
      this.fillColor = 'red'
    } else {
      //outside target
...
function targetHandle(curTrgt) {
  if (hair.x >= curTrgt.x && hair.x <= (curTrgt.x + curTrgt.s) && hair.y >= curTrgt.y && hair.y <= (curTrgt.y + curTrgt.s)) {
    //TARGET HIT!
    curTrgt.hit();
  } else {
    //TARGET MISS
...
function play(curTrgt, scn) { //sets up the PLAY scenes
